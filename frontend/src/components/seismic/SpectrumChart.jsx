/**
 * StructCalc — RPA 2024 Spectrum Visualizer (Session 9)
 *
 * Changes:
 *   - Two-direction export: when twoDir=true, exports Sad_x.txt AND Sad_y.txt
 *   - Single direction: exports one Sad.txt (unchanged)
 *   - Svd export always single (vertical is direction-independent)
 */
...
function ChartTooltip({active,payload,c}){
  ...
  return(
    <div style={{background:c.elevated,border:`1px solid ${c.borderLight}`,
      ...
      <div style={{color:c.textMuted}}>Sa/g = <b style={{color:c.green,fontSize:15}}>{d.Sa_g.toFixed(4)}</b></div>
    </div>
  )
}
...
function MiniChart({data,color,T1,T2,T3,floor,peak,label,eq,c}){
  return(
    <div style={{background:c.surface,border:`1px solid ${c.border}`,
      borderRadius:12,padding:"16px 12px 10px",flex:1,minWidth:240}}>
      <div style={{fontSize:12,color:c.textSec,marginBottom:10,display:"flex",
        justifyContent:"space-between",fontWeight:500}}>
        <span><b style={{color,fontWeight:700}}>{label}</b>&nbsp;·&nbsp;
          <span style={{color:c.blue}}>{eq}</span></span>
...
function exportTxt(pts, filename) {
  // Robot-compatible format: T and Sa/g only, no headers
  const lines = pts.map(p => p.T.toFixed(2).padEnd(10) + p.Sa_g.toFixed(6))
...
function ExportBtn({ label, color, onClick, c }) {
  return (
    <button type="button" onClick={onClick} style={{
      padding:"8px 13px",borderRadius:8,cursor:"pointer",
      background:color+"11",border:`1px solid ${color}66`,
...
export default function SpectrumChart({ params, c, isDark }) {
  const [specX,   setSpecX]   = useState(null)
  const [specY,   setSpecY]   = useState(null)
  const [specV,   setSpecV]   = useState(null)
  ...
  useEffect(()=>{
    ...
    async function fetchOne(QF, R, setFn) {
      const res = await fetch("http://localhost:8000/api/v1/spectrum",{
        ...
        body:JSON.stringify({
          zone, site_class:params.site, importance_group:params.group,
          QF, R, T_step:0.01
        }),
    ...
    async function fetchAll(){
      ...
      try{
        if(!params.twoDir){
          await fetchOne(params.QF, params.R, d=>{setSpecX(d);setSpecV(d)})
          setSpecY(null)
        } else {
          await Promise.all([
            fetchOne(params.QFx, params.Rx, d=>setSpecX(d)),
            fetchOne(params.QFy, params.Ry, d=>{setSpecY(d);setSpecV(d)}),
          ])
        }
      } catch(err){
    ...
    <div style={{background:c.bg,minHeight:"100vh",color:c.text,
      ...
      {apiErr&&(
        <div style={{background:c.red+"15",border:`1px solid ${c.red}44`,
          ...
          <span>❌</span><span style={{lineHeight:1.5}}>{apiErr}</span>
        </div>
      )}
      {loading&&!specX&&(
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
        ...
        {!params.twoDir
          ?<><Card label="QF" value={typeof params.QF==="number"?params.QF.toFixed(2):params.QF}
                unit="qualité" accent={c.amber} c={c}/>
      ...
      <div style={{background:c.surface,border:`1px solid ${c.border}`,
        ...
        {/* Formula trace */}
        <div style={{flex:1,fontSize:12,color:c.textSec,fontFamily:"monospace",minWidth:220}}>
          {!params.twoDir?(
            <>
              ...
              {"  "}Svd={<span style={{color:c.purple,fontWeight:700}}>{vd.peak}</span>}
            </>
          ):(
            <>
              <span style={{color:c.blue,fontWeight:700}}>X</span>{" "}Sad_x={hX.peak}{"  "}
              <span style={{color:c.purple,fontWeight:700}}>Y</span>{" "}Sad_y={hY.peak}{"  "}
              <span style={{color:c.green,fontWeight:700}}>V</span>{" "}Svd={vd.peak}
            </>
          )}
        </div>

        {/* Export buttons — adapt to direction mode */}
        <div style={{display:"flex",gap:8,flexShrink:0,flexWrap:"wrap"}}>
          <button type="button"
            onClick={()=>alert("Export vers Robot — Session connexion bridge")}
          ...
          {!params.twoDir?(
            <>
              <ExportBtn label="Sad → .txt" color={c.blue}
                onClick={()=>specX&&exportTxt(specX.hData.pts,
                  `RPA24_Sad_Zone${params.zone}_${params.site}.txt`)} c={c}/>
            </>
          ):(
            <>
              <ExportBtn label="Sad_x → .txt" color={c.blue}
                onClick={()=>specX&&exportTxt(specX.hData.pts,
                  `RPA24_Sad_X_Zone${params.zone}_${params.site}.txt`)} c={c}/>
              <ExportBtn label="Sad_y → .txt" color={c.purple}
                onClick={()=>specY&&exportTxt(specY.hData.pts,
                  `RPA24_Sad_Y_Zone${params.zone}_${params.site}.txt`)} c={c}/>
            </>
          )}
          <ExportBtn label="Svd → .txt" color={c.green}
            onClick={()=>specV&&exportTxt(specV.vData.pts,
              `RPA24_Svd_Zone${params.zone}_${params.site}.txt`)} c={c}/>
        </div>
      </div>
    </div>
  )
